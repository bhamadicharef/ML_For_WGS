...
eqCO=computeEqCO(dfxgb['Temperature (C)'].values,dfxgb['H2 vol.%'].values,
                 dfxgb['CO vol.%'].values,dfxgb['H2O vol.%'].values,
                 dfxgb['CO2 vol.%'].values)
...
allx=dfxgb[dfxgb.columns[:nb_xs]].to_numpy()
print(allx.shape)
allyX=dfxgb.loc[:,['CO_Conversion','Eq_CO_Conversion','CO_xgboost']].to_numpy()
...
#------------------------------------
columnpred=dfxgb.columns.to_list()+['CO_ann']+['CO_xgboost_ann']
predictionXsYs=np.concatenate((predictionXs.T,predictionYs.T),axis=0).T
#print(predictionXs.shape,predictionYs.shape, predictionXsYs.shape)
